...
export async function GET() {
  ...
  if (error) {
    return NextResponse.json<ApiError>(
      { error: "Failed to fetch reputation", detail: error.message },
      { status: 500 }
    );
  }

  // Aggregate per agent: average score, count of jobs completed.
  ...
  const { data: agents } = await supabaseAdmin.from("agents").select("agent_id, wallet_address");
